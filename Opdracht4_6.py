...
for filename in os.listdir(image_folder):
    if filename.endswith("MPT.tif"):  
        try:
            ...
            plt.savefig(os.path.join(output_folder_MPT_ndvi, f"ndvi_map_{filename}"))
            plt.close()

            plt.imshow(savi_result, cmap='YlGn', vmin=0, vmax=1)
            plt.title(f"SAVI Map - {filename}")
            plt.colorbar(label="SAVI Value")
            plt.savefig(os.path.join(output_folder_MPT_savi, f"savi_map_{filename}"))
            plt.close()
        ...
        try:
            ...
            plt.title(f"NDVI Map - {filename}")
            plt.colorbar(label="NDVI Value")
            plt.savefig(os.path.join(output_folder_MQS_ndvi, f"ndvi_map_{filename}"))
            plt.close()

            plt.imshow(savi_result, cmap='YlGn', vmin=0, vmax=1)
            plt.title(f"SAVI Map - {filename}")
            plt.colorbar(label="SAVI Value")
            plt.savefig(os.path.join(output_folder_MQS_savi, f"savi_map_{filename}"))
            plt.close()
